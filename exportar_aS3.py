...
BUCKET_NAME = 'actividad5clase'  # <- cambia esto
...
def exportar_a_s3():
    ...
    lineas = []
    lineas.append("=" * 70)
    lineas.append(f"REPORTE FINANZAS MEXICO - {datetime.now(timezone.utc).strftime('%Y-%m-%d %H:%M:%S')} UTC")
    lineas.append("=" * 70)
    ...
    for item in sorted(items, key=lambda x: x['id']):
        ...
        lineas.append(f"   Monto:    ${item.get('monto_venta', 0)}")
